...
# Update ECS task definition to use new AMD64 Ollama image
# This fixes the "exec format error" by using the correct architecture

set -e

echo "🔄 Updating ECS task definition to use AMD64 Ollama image..."

# Configuration
ECR_REPO_URI="216673926326.dkr.ecr.ap-southeast-1.amazonaws.com/words-wall-backend"
NEW_IMAGE_TAG="amd64-20250714-081257"  # The AMD64 image we just built
FULL_IMAGE_URI="$ECR_REPO_URI:$NEW_IMAGE_TAG"

echo "📋 Configuration:"
echo "  New Image: $FULL_IMAGE_URI"
echo "  Memory: 4096 MB (for Ollama + Qwen model)"
echo "  CPU: 2048 (for AI processing)"
echo "  Architecture: linux/amd64 (compatible with AWS Fargate)"
echo ""

# Create new task definition with updated image and resources
echo "📝 Creating new task definition..."

cat > ollama-amd64-task-definition.json << EOF
{
...
NEW_TASK_DEF_ARN=$(aws ecs register-task-definition --cli-input-json file://ollama-amd64-task-definition.json --query 'taskDefinition.taskDefinitionArn' --output text)
...
# Cleanup
rm -f ollama-amd64-task-definition.json
